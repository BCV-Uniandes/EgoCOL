...
if __name__=='__main__':
    ...
    if args.visualize:
        visualization_folder = "visualizations_filtered" if args.filter else "visualizations"
        if not os.path.exists(os.path.join(visualization_folder,"common_by_scan")):
            os.makedirs(os.path.join(visualization_folder,"common_by_scan"))
        if not os.path.exists(os.path.join(visualization_folder,"common_by_scans_colors")):
            os.makedirs(os.path.join(visualization_folder,"common_by_scans_colors"))
        if not os.path.exists(os.path.join(visualization_folder,"all_by_scan")):
            os.makedirs(os.path.join(visualization_folder,"all_by_scan"))
        if not os.path.exists(os.path.join(visualization_folder,"colmap_relative_by_scan")):
            os.makedirs(os.path.join(visualization_folder,"colmap_relative_by_scan"))
    ...
    for scan in extr_dict_colmap_total:
        ...
        if args.constrain and len(extr_dict_pnp)>0:
            root = os.path.join(args.path_to_scans,"_".join(scan.split("_")[:3]))
        ...
        if args.visualize:
            #ply colmap relative cooridnate system
            datos = []
            for frame in extr_dict_colmap:
                center = camera_center_from_extrinsics(extr_dict_colmap[frame])
                datos.append(center)
            datos = np.array(datos)
            export_to_pyl(datos, True, os.path.join(visualization_folder,"colmap_relative_by_scan",scan+".ply"))
        ...
        if T_proc is None:
            print("NO T_proc")
            continue
        T_procs_dict[scan] = T_proc

        valid_frames += len(extr_dict_colmap)
        centers_colmap_onscan = transform_points(centers_colmap, T_proc)

        if args.visualize:
            export_to_pyl(centers_colmap_onscan[0:3,:], True, os.path.join("visualizations_filtered","common_by_scan",scan+"_colmap.ply"))
            export_to_pyl(centers_pnp, False,  os.path.join("visualizations_filtered","common_by_scan",scan+"_pnp.ply"))
        ...
        for clip in clips:
            if os.path.exists(os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "valid_poses_colmap_by_scan_filtered.npy")):
                os.remove(os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "valid_poses_colmap_by_scan_filtered.npy"))
            if os.path.exists(os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "poses_colmap_by_scan_filtered.npy")):
                os.remove(os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "poses_colmap_by_scan_filtered.npy"))
            ...
            for i in tqdm.tqdm(range(len(os.listdir(os.path.join(path_to_clips_frames, clip))))):
                if os.path.join(clip,'color_%07d.jpg'%i) in extr_dict_colmap:
                    if args.constrain:
                        pose = constraint_inside_and_relocate(extr_dict_colmap[os.path.join(clip,'color_%07d.jpg'%i)] ,mesh, scene)
            ...
            path_to_save_valid_poses = os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "valid_poses_colmap_by_scan_filtered.npy")
            path_to_save_poses = os.path.join(root_dir, clip, "egovideo",'superglue_track', 'poses', "poses_colmap_by_scan_filtered.npy")
            ...
            if args.visualize:
                ...
                if len(datos)==0:
                    continue
                datos = np.array(datos)
                export_to_pyl(datos, True,  os.path.join(visualization_folder,"all_by_scan",clip+"_colmap.ply"))
        ...
        if args.visualize:
            ...
            for frame in extr_dict_pnp:
                ...
                datos.append(center)
            datos = np.array(datos)
            export_to_pyl(datos, True,os.path.join(visualization_folder,"all_by_scan",clip+"pnp.ply"))
    if args.split=="val":
        print("Clips: {}/{} ({}), Frames {}/{} ({})".format(valid_clips, 44, valid_clips*100/44, valid_frames, 96039, valid_frames*100/96039))
    else:
        print("Clips: {}/{} ({}), Frames {}/{} ({})".format(valid_clips, 69, valid_clips*100/69, valid_frames, 158023, valid_frames*100/158023))