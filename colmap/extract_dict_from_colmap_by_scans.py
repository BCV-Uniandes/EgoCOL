...
def extract_dict(args):
    ...
    scans = os.listdir(args.input_dir_colmap)
    i=1
    for scan in scans:
        total_poses[scan] = {}
        for clip in os.listdir(os.path.join(args.input_dir_colmap,scan,"database")):
            total_images+= len(os.listdir(os.path.join(args.input_dir_colmap,scan,"database",clip)))
        if exists(os.path.join(args.input_dir_colmap,scan,"sparse/reg/images.txt")):
            with open(os.path.join(args.input_dir_colmap,scan,"sparse/reg/images.txt"), "r") as f:
                poses =  f.readlines()[4:]
            for pose in tqdm.tqdm(poses):
                if "jpg" in pose:
                    ...
                    total_poses[scan][params[-1].strip("\n")] = extrinsics
                    total_registered+=1
                    
    with open('colmap_ext_{}_scan.pkl'.format(args.split), 'wb') as f:
