...
def save_crop(clip_uid,qset):
    root = "/media/SSD5/ego4d/dataset/3d/v1/clips_5fps_frames"
    object_title = qset["object_title"]
    crop = qset["visual_crop"]
    img = Image.open(os.path.join(root, clip_uid,'color_%07d'%crop["frame_number"]+".jpg"))
    visual_crop =  img.crop((crop["x"], crop["y"], crop["x"]+crop["width"], crop["y"]+crop["height"]))
    visual_crop.save(os.path.join("/media/SSD5/ego4d/dataset/3d/v1/visualizations/Crops", "{}_{}.png".format(object_title, clip_uid)))
