...
root_folder_dirs = os.listdir(ROOT_FOLDER)
root_folder_dirs.sort()
for img_idx in tqdm(range(len(fnmatch.filter(root_folder_dirs, '*.jpg')))):
    image = Image.open(os.path.join(ROOT_FOLDER, 'color_%07d.jpg' % int(root_folder_dirs[img_idx].split("_")[1].replace(".jpg",""))))
    ...
    undistorted_image.save(os.path.join(OUT_FOLDER, 'color_%07d.jpg' % int(root_folder_dirs[img_idx].split("_")[1].replace(".jpg",""))))
